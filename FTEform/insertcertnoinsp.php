...
if(mysqli_connect_error()) {
    ...
    $UPDATEMINE = "UPDATE ftedata SET cert = ? WHERE orden = ?";

    $UPDATEINSP = "UPDATE ftecp, ftedata SET 
    ftecp.airplanec = ftedata.airplane,
    ftecp.inspc = ftedata.insp,
    ftecp.datosc = ftedata.datos,
    ftecp.estadoc = ftedata.estado
    ...
    $UPDATEINSP2 = "UPDATE ftedata, ftecp SET 
    ftedata.airplane = ftecp.airplanec,
    ftedata.insp = ftecp.inspc,
    ftedata.datos = ftecp.datosc,
    ftedata.estado = ftecp.estadoc
    ...
    $COM3 = "SELECT orden, ordenc FROM ftedata, ftecp WHERE orden = ordenc AND cert != ' '"; 
    ...
    if($id != '' && $care == 0 && $care4 != 0) {
        ...
        if($care3 == 0 && $amicertif == 'yes') {
    		...
    		$result->bind_param("sssiisiisiisiisiis", $id, $orden, $fecha, 
    		$hs1, $he1, $datos1, $hs2, $he2, $datos2, $hs3, $he3, $datos3, $hs4, $he4, $datos4, 
    		$hs5, $he5, $datos5);
    		$result->execute();
       		$result->close();

            $result = $mysqli->prepare($UPDATEMINE);
            $result->bind_param("ss", $cert, $orden);
            $result->execute();
            $result->close();

            $result = $mysqli->prepare($UPDATEINSP);
        	$result->execute();
        	$result->close();

        	$result = $mysqli->prepare($UPDATEINSP2);
        	$result->execute();
        	$result->close();
    	}
    	else {
